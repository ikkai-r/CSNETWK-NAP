...
public class Client {
        ...
        try {
            while (isRunning) {
                ...
                if (validateInput(inputArr)) {
                    ...
                    if (command.equals("/leave") || command.equals("/get") || command.equals("/store")
                            || command.equals("/dir") || command.equals("/register") || command.equals("/?")) {
                        receiveFileMsg(command, clientSocket, disReader, dosWriter, input, inputArr, greeting);
                    } else {
                        switch (command) {
                            case "/join" -> {
                                try {
                                    listenMsgServer(disReader);
    ...
    private static void sendFile(String fileName, DataInputStream disReader, DataOutputStream dosWriter)
            throws IOException {
    ...
    private static void receiveFileMsg(String command, Socket clientSocket, DataInputStream disReader,
            DataOutputStream dosWriter, String input, String[] inputArr, String greeting) throws IOException {
        switch (command) {
            case "/leave" -> {
                if (clientSocket == null) {
    ...
    private static void listenMsgServer(DataInputStream disReader) {
        Thread messageReceiverThread = new Thread(() -> {
            try {
                while (true) {
                    String receivedMessage = disReader.readUTF();

                    if (receivedMessage.startsWith("/BCAST/")) {
                        System.out.println(disReader.readUTF());
                    }
                }
            } catch (IOException e) {
